...
| Text node                                                         | `text`                                                               | `tag` by hierarchy (largest heading `h1`, then `h2`, `h3`, body `p`, inline `span`, link `a` + `href`); `content` as `<p>…</p>` (bold runs → `<strong>`); `fontSize` px, `fontWeight` (400/500/600/700), `fontFamily` + `fontUrl` when it is a workspace font, `lineHeight` (px or unitless), `letterSpacing` px, `color`, `textAlign`; a fixed-width text box → `width` (Jule applies it as `max-width`) |
...
- Fonts that are not in the workspace branding render with the visitor's fallback unless the
  design's font is web-available and you provide `fontUrl`.
- Images and icons need hosted URLs or library entries; MCP v1 has no upload.
