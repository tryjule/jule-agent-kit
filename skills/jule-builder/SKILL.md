...
- Every further page that collects answers ends with one button: "next", or "submit" on the last one. Submit once per visit: never a submit before a page that still collects answers (those answers would be lost); a thank-you or coupon page may follow it. The editor's six actions are next, previous, submit, submit_and_redirect, close and redirect; submit_and_next, submit_and_page and skip are legacy and never written.
...
- style_config.display_mode "whole_page", card_width "100%", card_padding "0px"; the navbar and footer are chrome, not sections — linked to branding (preference_center_header_linked / preference_center_footer_linked) or written in preference_center_header_html / _css / _js and the footer fields, never rebuilt out of Boxes and text items; written chrome copies the template under "Header and footer HTML" in jule://docs/project-style (menu toggle in _js, breakpoints at 820 and 480px, no fixed widths) and is measured at 390px by the rendered review; SEO fields filled (seo_page_title, seo_meta_description, seo_og_image_url).
- Every clickable thing is a button item: a link drawn as a pill, a short centred label on a background, a Box holding a label and an arrow — never a text item. Repeated cards (features, steps, team) share one structure and one set of sizes: build one, copy it, change the words.
